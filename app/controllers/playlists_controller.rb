class PlaylistsController < ApplicationController
  before_action :find_playlist, only: %i[update delete]
  def all_playlists
  ...
  def add_songs
    ...
    render json: { message: 'playlists not present' } and return unless playlist.present?
  ...
  def show
    debugger
    @playlist = @current_user.playlists.find_by(id: params[:id])
    render json: { message: 'Playlists not presents' }, status: :bad_request unless @playlist
