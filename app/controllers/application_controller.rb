class ApplicationController < ActionController::API
  ...
  def permit_action
    return if @current_user.role.to_i == 1
  ...
  def authenticate_user!
    unless request.headers['Authorization'].present?
      render json: { status: { code: '401', message: 'Unauthorized Request' } },
             status: :unauthorized and return
    end

    jwt_payload = JWT.decode(request.headers['Authorization'].split(' ')[1],
                             Rails.application.credentials.fetch(:devise_jwt_secret_key), false)&.first
    @current_user = User.find_by(jti: jwt_payload['jti'])
    return if @current_user.present?

    render json: { status: { code: '401', message: I18n.t('Invalid token') } },
           status: :unauthorized and return
  end
end
