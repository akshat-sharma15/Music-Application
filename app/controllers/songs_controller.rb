class SongsController < ApplicationController
  before_action :permit_action, only: %i[update delete create]
  def allsongs
    songs = Song.all
    if songs
      songs_with_urls = songs.map do |song|
        song.as_json.merge(audio_url: song.audio.attached? ? url_for(song.audio) : nil,
  ...
  def update
    ...
    if song
      render json: song.as_json.merge(audio_url: url_for(song.audio),
                                      poster_url: url_for(song.poster))
    else
  ...
  def create
    ...
    if song.present?
      render json: { song: song, song_url: url_for(song.audio), poster_url: url_for(song.poster) },
             status: :ok
    else
      render json: song.errors, status: :unprocessable_entity
    end
